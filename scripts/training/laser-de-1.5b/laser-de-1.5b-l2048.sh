...
NNODES=1 # Recommended to use 2/4 nodes

export PROJECT_NAME=Laser-Training
export WANDB_API_KEY="YOUR_WANDB_API_KEY"
export VLLM_ATTENTION_BACKEND=XFORMERS
export HDFS_DATA_PATH=$DATAPATH
export HDFS_CHECKPOINT_PATH=$MODEL_PATH
export HDFS_LOG_PATH="$HDFS_DATA_PATH/logs"
export RUN_NAME=Laser-DE-1.5B-L2048

# Default values
# echo "WARNING: This is a development script. The batch size is set to 128. Length is set to 4096."
...
length_reward_scalar=0.5
target_length=8192  # this parameter is not used in this script

TARGET_LENGTH_UPPER_BOUND=$MAX_RESPONSE_LENGTH
TARGET_LENGTH_LOWER_BOUND=2048
TARGET_LENGTH_INTERVAL=1024
...
generate_suffix() {
  ...
  while [[ "$#" -gt 0 ]]; do
    case $1 in
      ...
      --ppo_micro_batch_size) suffix+="_ppomicro$2"; shift 2 ;;
...
echo -e "Training with the following parameters:\nTrain Batch Size: $TRAIN_BATCH_SIZE\nVal Batch Size: $VAL_BATCH_SIZE\nMax Prompt Length: $MAX_PROMPT_LENGTH\nMax Response Length: $MAX_RESPONSE_LENGTH\nLearning Rate: $LEARNING_RATE\nPPO Mini Batch Size: $PPO_MINI_BATCH_SIZE\nPPO Micro Batch Size: $PPO_MICRO_BATCH_SIZE\nKL Loss Coefficient: $KL_LOSS_COEF\nKL Loss Type: $KL_LOSS_TYPE\nTemperature: $TEMPERATURE\nRollout N: $ROLLOUT_N\nKL Coefficient: $KL_COEF\nTotal Epochs: $TOTAL_EPOCHS\nDataset Name: $DATASET_NAME\nModel Name: $MODEL_NAME" >> $LOG_FILE_PATH
sleep 3
PYTHONUNBUFFERED=1 python -m verl.trainer.main_ppo \
    length_penalty.enable=True \
    length_penalty.method=larse-de \
    length_penalty.length_reward_scalar=$length_reward_scalar \
    length_penalty.target_length=$target_length \
    length_penalty.upper_bound=$TARGET_LENGTH_UPPER_BOUND \
    length_penalty.lower_bound=$TARGET_LENGTH_LOWER_BOUND \
    length_penalty.interval=$TARGET_LENGTH_INTERVAL \
    algorithm.adv_estimator=grpo \
    trainer.remove_previous_ckpt=$REMOVE_PREVIOUS_CKPT \
    data.train_files=$HDFS_DATA_PATH/data/deepscaler/train.parquet \
    data.val_files=$HDFS_DATA_PATH/data/deepscaler/aime.parquet \
    data.monitor_files=$HDFS_DATA_PATH/data/deepscaler/train_sample500.parquet \
